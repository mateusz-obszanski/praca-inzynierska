...
def graph_cycle_greedy_nn(cost_mx: np.ndarray, initial_vx: int) -> list[int]:
    ...
    def register_neighbours_by_distance(vx: int):
        ...
        reachable_sorted_neighbours = sorted(n for d, n in zip(neighbour_distances, vxs) if d > 0)
        reachable_sorted_neighbours = [n for d, n in sorted((d, n) for d, n in zip(neighbour_distances, vxs) if d > 0)]
    ...
    while i < vx_num and i > -1:
        ...
        neighbours_to_visit = [n for n in neighbours_by_distance[current_vx] if n not in solution]

        try:
            next_to_visit = next(n for n in neighbours_to_visit if n not in visited_neighbours[current_vx])
